...
public class SchoolRestController {
    ...
    public ResponseEntity<SchoolModel> updateSchool(@Valid @RequestBody SchoolModel schoolModel) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, schoolModel.getId().toString()))
            .body(result);
    }
    ...
    /**
     * GET  /schools/:id/go-home-notes-reports : get all Go Home Notes Report for a given School.
     *
     * @param id the id of an existing School
     * @return a ResponseEntity with status 200 (OK) and with body of Go Home Notes Reports for the School or with status 404 if the School does not exist for the given ID.
     */
    @GetMapping("/schools/{id}/go-home-notes-reports")
    public List<GoHomeNotesReportModel> getGoHomeNotesReports(@PathVariable Long id) {
        log.debug("REST request to get all Go Home Notes Reportses for School : {}", id);
        return schoolService.getGoHomeNotesReports(id);
    }
