...
export class StudentService {
    ...
    query(req?: any): Observable<HttpResponse<IStudent[]>> {
        ...
        return this.http
            .get<IStudent[]>(`api/schools/${schoolId}/students`, { params: options, observe: 'response' })
            .pipe(map((res: HttpResponse<IStudent[]>) => this.deriveIdentifierFromArray(res)));
    }
    ...
    deriveIdentifierFromArray(res: HttpResponse<IStudent[]>): HttpResponse<ITeacher[]> {
        res.body.forEach((student: IStudent) => {
            student.identifier = student.lastName + ', ' + student.firstName;
        });
        return res;
    }
}
